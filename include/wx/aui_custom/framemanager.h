...
class wxAuiMarkWindow:wxFrame
{
	friend class wxAuiDockMarker;
public:
	wxAuiMarkWindow(wxWindow* pParent,MarkPosition mark_pos);
...
private:
	wxBitmap m_bmp;
	wxBitmap m_btmSelected;
	MarkPosition m_mark_pos;
	bool m_bSel;
wxDECLARE_EVENT_TABLE();
};

class wxAuiMarkGroupWindow:wxFrame
{
	friend class wxAuiDockMarker;
public:
	wxAuiMarkGroupWindow(wxWindow* pParent);
	void Select(MarkPosition mark_pos,bool bSel=true);
	bool GetSel(MarkPosition& mark_pos);
	bool HitTest(const wxPoint &pt,MarkPosition& mark_pos);
private:
	void OnPaint(wxPaintEvent& evt);
	void OnMotion(wxMouseEvent& evt);
	void OnEraseBkg(wxEraseEvent &evt);
...
private:
	wxBitmap m_bmp_left;
	wxBitmap m_bmp_left_h;
	wxBitmap m_bmp_top;
	wxBitmap m_bmp_top_h;
	wxBitmap m_bmp_right;
	wxBitmap m_bmp_right_h;
	wxBitmap m_bmp_bottom;
	wxBitmap m_bmp_bottom_h;
	wxBitmap m_bmp_middle;
	wxBitmap m_bmp_middle_h;
	wxBitmap m_bmp_base;

	wxRegion m_rg_left;
	wxRegion m_rg_top;
	wxRegion m_rg_right;
	wxRegion m_rg_bottom;
	wxRegion m_rg_middle;
	wxRegion m_rg_base;
...
{
public:
	wxAuiDockMarker(wxAuiManager* pMgr);
	~wxAuiDockMarker();

	void Show(bool bShow = true,bool bShowGroup = true);
	void SetContainer(wxWindow* pContainer,bool bShowMiddle=true,bool isMain=false,wxRect rc=wxRect());
	void GetDockMark(wxWindow* &pContainer,bool& bMainframe, bool& bDock, MarkPosition& mark_pos);
	void HitTest(const wxPoint& pt,wxWindow* &pContainer,bool& bMainframe, bool& bDock, MarkPosition& mark_pos,bool &isCenter,bool bRefresh = true);
private:
	wxAuiManager *m_pMgr;
	wxWindow* m_pContainer;
	wxAuiMarkWindow* m_arrMarkWnd[4];
	wxAuiMarkGroupWindow *m_pMarkGroupWnd;
};
...
public:
	...
	//tab页管理信息
	wxNotebookEx* notebook;	// notebook for tab page manage
	
	bool HasTab(){return notebook?true:false;};

    bool IsValid() const;
};

class wxNotebookEx:public wxNotebook
{
public:
	wxNotebookEx(wxWindow* parent,wxAuiManager* mgr);
	void SetPaneInfo(wxAuiPaneInfo& pane){pane_info = &pane;};
	bool AddPage(wxWindow *page, const wxString& text, wxAuiPaneInfo& pane, bool bSelect = false, int imageId = NO_IMAGE);
...
public:
    ...
    virtual wxAuiFloatingFrame* CreateFloatingFrame(wxWindow* parent, const wxAuiPaneInfo& p,wxAuiManager* owner_mgr=NULL);
...
public:
	void SetMainManager(bool isMain){m_bIsmainMgr = isMain;};
