...
#include "wx/aui_custom/tabmdi.h"
#include "wx/aui_custom/auibar.h"
#include "wx/mdi.h"
...
static void DoInsertDockRowEx(wxAuiPaneInfoArray& panes,
							int dock_direction,
							int dock_layer,
							int dock_row,
							int dock_pos)
													  ...
													  wxAuiManager* owner_mgr)
{
	...
	return new wxAuiFloatingFrame(parent, owner_mgr==NULL?this:owner_mgr, paneInfo);
}
...
void wxAuiManager::SetManagedWindow(wxWindow* wnd)
{
	...
	if(m_bIsmainMgr)
		m_pDockMarker = new wxAuiDockMarker(this);
...
void wxAuiManager::Update()
{
	...
	if(pane_count==1)
	{
		wxAuiFloatingFrame *pFrame = wxDynamicCast(m_frame,wxAuiFloatingFrame);
		if(pFrame)
		{
			pFrame->m_paneWindow = m_panes[0].window;
			m_panes[0].Center().CaptionVisible(false).PaneBorder(false);
		}
	}
    ...
    for (i = 0; i < pane_count; ++i)
    {
        ...
        if (p.IsFloating())
        {
            if (p.frame == NULL)
            {
				wxAuiFloatingFrame* mgr_frame = wxDynamicCast(m_frame,wxAuiFloatingFrame);
				if(mgr_frame!=NULL)
				{
					wxAuiManager *powner_mgr = mgr_frame->GetOwnerManager();
					wxAuiFloatingFrame* frame = CreateFloatingFrame(powner_mgr->GetManagedWindow(), p, powner_mgr);
					...
					wxAuiPaneInfo& pane_in_owner = powner_mgr->GetPane(p.window);

					pane_in_owner.frame = frame;
					DetachPane(p.window);
					pane_count = m_panes.GetCount();

					if (pane_in_owner.IsShown() && !pane_in_owner.frame->IsShown())
						pane_in_owner.frame->Show();
				}
				else
				{
...
void wxAuiManager::OnFloatingPaneMoveStart(wxWindow* wnd)
{
	...
	wxAuiFloatingFrame *pFrame = wxDynamicCast(wnd,wxAuiFloatingFrame);
	if(pFrame)
	{
		m_pDockMarker->Show(true,true);
		return;
	}
	...
	if(!pane.IsToolbar() && m_bIsmainMgr)
		m_pDockMarker->Show(true,true);
...
void wxAuiManager::OnFloatingPaneMoving(wxWindow* wnd, wxDirection dir)
{
	...
	wxAuiFloatingFrame *pFrame = wxDynamicCast(wnd,wxAuiFloatingFrame);
	wxAuiPaneInfo& pane = GetPane(wnd);
    wxASSERT_MSG(pane.IsOk() || pFrame, wxT("Pane window not found"));

    if(pane.frame==NULL && pFrame==NULL)
        return;
	...
	wxPoint frame_pos = pFrame==NULL?pane.frame->GetPosition():pFrame->GetPosition();
    wxPoint action_offset(pt.x-frame_pos.x, pt.y-frame_pos.y);

    // no hint for toolbar floating windows
    if (pFrame==NULL && pane.IsToolbar() && m_action == actionDragFloatingPane)
    {
        wxAuiDockInfoArray docks;
        wxAuiPaneInfoArray panes;
        wxAuiDockUIPartArray uiparts;
    ...
    // if a key modifier is pressed while dragging the frame,
    // don't dock the window
    if (pFrame == NULL && (!pane.IsOk() && !CanDockPanel(pane)))
    {
        HideHint();
        return;
    }
	...
	{
		// 调整里层停靠锚点提示窗口显示位置
		int i=0,cnt=m_panes.size();
		wxRect blank_rc = m_frame->GetClientRect();
		wxPoint blank_lt = m_frame->ClientToScreen(blank_rc.GetLeftTop());
		blank_rc = wxRect(blank_lt,blank_rc.GetSize());
		wxAuiPaneInfoPtrArray pipa;
		// 1) 检查所有浮动窗口
		for(;i<cnt;i++)
		{
			wxAuiPaneInfo &info = m_panes[i];
			if(info.IsOk()
				&& info.IsShown() 
				&& !info.IsToolbar()
				/*&& info.window->GetScreenRect().Contains(pt)*/
				/*&& !info.window->IsKindOf(wxCLASSINFO(wxAuiNotebook))*/)
			{
				if(info.frame && info.IsFloating()
					&& info.window->GetScreenRect().Contains(pt))
				{
					m_pDockMarker->SetContainer(info.window,true/*info.window->IsKindOf(wxCLASSINFO(wxAuiNotebook))?false:true*/);
					break;
				}
				else if(info.IsDocked())
				{
					pipa.Add(&info);
					if(!blank_rc.IsEmpty())
					{					
						wxRect itst_rc = info.window->GetScreenRect();
						switch(info.dock_direction)
						{
						case wxAUI_DOCK_TOP:
							blank_rc = wxRect(wxPoint(blank_rc.x,itst_rc.GetBottom()),blank_rc.GetRightBottom());
							break;
						case wxAUI_DOCK_RIGHT:
							blank_rc = wxRect(blank_rc.GetLeftTop(),wxPoint(itst_rc.x,blank_rc.GetBottom()));
							break;
						case wxAUI_DOCK_BOTTOM:
							blank_rc = wxRect(blank_rc.GetLeftTop(),wxPoint(blank_rc.GetRight(),itst_rc.y));
							break;
						case wxAUI_DOCK_LEFT:
							blank_rc = wxRect(wxPoint(itst_rc.GetRight(),blank_rc.y),blank_rc.GetRightBottom());
							break;
						case wxAUI_DOCK_CENTER: blank_rc = wxRect();break;
						}
					}
				}
			}
		...
		if(i == cnt)
		{
			for(i=0,cnt=pipa.GetCount();i<cnt;i++)
			{
				wxAuiPaneInfo *pInfo = pipa[i];
				if(pInfo->window->GetScreenRect().Contains(pt))
				{
					m_pDockMarker->SetContainer(pInfo->window,pInfo->window->IsKindOf(wxCLASSINFO(wxAuiNotebook))?false:true);
					break;
				}
			}
		}
		...
		if(i==cnt)
		{
			if(m_frame->GetClientRect().Contains(client_pt) && !blank_rc.IsEmpty())
				m_pDockMarker->SetContainer(m_frame,/*false*/true,true,blank_rc);
			else
				m_pDockMarker->SetContainer(NULL);
		}
		// 更新选中锚点显示状态
		wxWindow* pContainer = NULL;
		bool bMainFrame = false,bDock = false,bCenter = false;
		MarkPosition mp;
		m_pDockMarker->HitTest(pt,pContainer,bMainFrame,bDock,mp,bCenter);
	}

// qiugan comment 20190905
...
void wxAuiManager::OnFloatingPaneMoved(wxWindow* wnd, wxDirection dir)
{
    ...
    // if a key modifier is pressed while dragging the frame,
    // don't dock the window
    if (pane.IsOk() && CanDockPanel(pane) && pane.IsToolbar())
    {
	...
	if(m_bIsmainMgr)
	{
		...
		bool bMainFrame = false,bDock = false,bCenter = false;
		MarkPosition mp;
		m_pDockMarker->HitTest(pt,pContainer,bMainFrame,bDock,mp,bCenter,false);
		m_pDockMarker->Show(false,false);
		...
		{		
			if(bMainFrame)
			{
				...
				switch(mp)
				{
				case MarkPosition::MarkPosition_LEFT:
					if(bCenter)
					{
						pane.Dock().Direction(wxAUI_DOCK_LEFT).Layer(0).Row(GetMaxRow(m_panes,wxAUI_DOCK_LEFT,0)+1).Position(0).Show();
					}
					else
					{
						_layer = wxMax(wxMax(GetMaxLayer(m_docks,wxAUI_DOCK_LEFT)
						,GetMaxLayer(m_docks,wxAUI_DOCK_TOP))
						,GetMaxLayer(m_docks,wxAUI_DOCK_BOTTOM)) + 1;
						pane.Dock().Direction(wxAUI_DOCK_LEFT).Layer(_layer).Row(0).Position(0).Show();
					}
					break;
				case MarkPosition::MarkPosition_TOP:
					if(bCenter)
					{
						pane.Dock().Direction(wxAUI_DOCK_TOP).Layer(0).Row(GetMaxRow(m_panes,wxAUI_DOCK_TOP,0)+1).Position(0).Show();
					}
					else
					{
						_layer = wxMax(wxMax(GetMaxLayer(m_docks,wxAUI_DOCK_TOP)
						,GetMaxLayer(m_docks,wxAUI_DOCK_LEFT))
						,GetMaxLayer(m_docks,wxAUI_DOCK_RIGHT)) + 1;
						pane.Dock().Direction(wxAUI_DOCK_TOP).Layer(_layer).Row(0).Position(0).Show();
					}
					break;
				case MarkPosition::MarkPosition_RIGHT:
					if(bCenter)
					{
						pane.Dock().Direction(wxAUI_DOCK_RIGHT).Layer(0).Row(GetMaxRow(m_panes,wxAUI_DOCK_RIGHT,0)+1).Position(0).Show();
					}
					else
					{
						_layer = wxMax(wxMax(GetMaxLayer(m_docks,wxAUI_DOCK_RIGHT)
							,GetMaxLayer(m_docks,wxAUI_DOCK_TOP))
							,GetMaxLayer(m_docks,wxAUI_DOCK_BOTTOM)) + 1;
						pane.Dock().Direction(wxAUI_DOCK_RIGHT).Layer(_layer).Row(0).Position(0).Show();
					}
					break;
				case MarkPosition::MarkPosition_BOTTOM:
					if(bCenter)
					{
						pane.Dock().Direction(wxAUI_DOCK_BOTTOM).Layer(0).Row(GetMaxRow(m_panes,wxAUI_DOCK_BOTTOM,0)+1).Position(0).Show();
					}
					else
					{
						_layer = wxMax(wxMax(GetMaxLayer(m_docks,wxAUI_DOCK_BOTTOM)
							,GetMaxLayer(m_docks,wxAUI_DOCK_LEFT))
							,GetMaxLayer(m_docks,wxAUI_DOCK_RIGHT)) + 1;
						pane.Dock().Direction(wxAUI_DOCK_BOTTOM).Layer(_layer).Row(0).Position(0).Show();
					}
					break;
				case MarkPosition::MarkPosition_MIDDLE:/* amd = wxAUI_DOCK_CENTER; */
					if(bCenter)
					{
						pane.Dock().Direction(wxAUI_DOCK_CENTER).Layer(0).Row(0).Position(0).Show();
					}
					break;
				}
			}
			...
			{
				wxAuiPaneInfo &pane_container = GetPane(pContainer);
				int layer = pane_container.dock_layer;
				int pos = pane_container.dock_pos;
				int row =pane_container.dock_row;
				wxAuiManagerDock pane_amd = (wxAuiManagerDock)pane_container.dock_direction;
				if(pane_container.frame && pane_container.IsFloating())
				{
					wxAuiFloatingFrame *pFrame = wxDynamicCast(pane_container.frame,wxAuiFloatingFrame);
					pane.window->Reparent(pFrame);
					...
					{
						case MarkPosition::MarkPosition_LEFT:
							pFrame->SetPaneWindow(pane,wxAUI_DOCK_LEFT);
							pane.frame = pFrame;
							pane.Float();
							break;
						case MarkPosition::MarkPosition_TOP:
							pFrame->SetPaneWindow(pane,wxAUI_DOCK_TOP);
							pane.frame = pFrame;
							pane.Float();
							break;
						case MarkPosition::MarkPosition_RIGHT:
							pFrame->SetPaneWindow(pane,wxAUI_DOCK_RIGHT);
							pane.frame = pFrame;
							pane.Float();
							break;
						case MarkPosition::MarkPosition_BOTTOM:
							pFrame->SetPaneWindow(pane,wxAUI_DOCK_BOTTOM);
							pane.frame = pFrame;
							pane.Float();
							break;
						case MarkPosition::MarkPosition_MIDDLE:
							pFrame->SetPaneWindow(pane,wxAUI_DOCK_CENTER,pane_container.window);
							break;
					}
					...
					case MarkPosition::MarkPosition_LEFT:
						{
							...
							{
							case wxAUI_DOCK_TOP:
								DoInsertPane(m_panes,pane_amd,layer,row,pos);
								pane.Dock().Direction(wxAUI_DOCK_TOP).Layer(layer).Row(row).Position(pos).Show();
								break;
							case wxAUI_DOCK_RIGHT:
								DoInsertDockRowEx(m_panes,pane_amd,layer,row+1,pos);
								pane.Dock().Direction(wxAUI_DOCK_RIGHT).Layer(layer).Row(row+1).Position(0).Show();
								break;
							case wxAUI_DOCK_BOTTOM:
								DoInsertPane(m_panes,pane_amd,layer,row,pos);
								pane.Dock().Direction(wxAUI_DOCK_BOTTOM).Layer(layer).Row(row).Position(pos).Show();
								break;
							case wxAUI_DOCK_LEFT:
								DoInsertDockRowEx(m_panes,pane_amd,layer,row,pos);
								pane.Dock().Direction(wxAUI_DOCK_LEFT).Layer(layer).Row(row).Position(0).Show();
								break;
							case wxAUI_DOCK_CENTER:
								pane.Dock().Direction(wxAUI_DOCK_LEFT).Layer(0).Row(GetMaxRow(m_panes,wxAUI_DOCK_LEFT,0)+1).Position(0).Show();
								break;
					...
					case MarkPosition::MarkPosition_TOP:
						{
							...
							case wxAUI_DOCK_TOP:
								DoInsertDockRowEx(m_panes,pane_amd,layer,row,pos);
								pane.Dock().Direction(wxAUI_DOCK_TOP).Layer(layer).Row(row).Position(0).Show();
								break;
							case wxAUI_DOCK_RIGHT:
								DoInsertPane(m_panes,pane_amd,layer,row,pos);
								pane.Dock().Direction(wxAUI_DOCK_RIGHT).Layer(layer).Row(row).Position(pos).Show();
								break;
							case wxAUI_DOCK_BOTTOM:
								DoInsertDockRowEx(m_panes,pane_amd,layer,row+1,pos);
								pane.Dock().Direction(wxAUI_DOCK_BOTTOM).Layer(layer).Row(row+1).Position(0).Show();
								break;
							case wxAUI_DOCK_LEFT:
								DoInsertPane(m_panes,pane_amd,layer,row,pos);
								pane.Dock().Direction(wxAUI_DOCK_LEFT).Layer(layer).Row(row).Position(pos).Show();
								break;
							case wxAUI_DOCK_CENTER:
								pane.Dock().Direction(wxAUI_DOCK_TOP).Layer(0).Row(GetMaxRow(m_panes,wxAUI_DOCK_TOP,0)+1).Position(0).Show();
								break;
					...
					case MarkPosition::MarkPosition_RIGHT:
						{
							...
							{
							case wxAUI_DOCK_TOP:
								DoInsertPane(m_panes,pane_amd,layer,row,pos+1);
								pane.Dock().Direction(wxAUI_DOCK_TOP).Layer(layer).Row(row).Position(pos+1).Show();
								break;
							case wxAUI_DOCK_RIGHT:
								DoInsertDockRowEx(m_panes,pane_amd,layer,row,pos);
								pane.Dock().Direction(wxAUI_DOCK_RIGHT).Layer(layer).Row(row).Position(0).Show();
								break;
							case wxAUI_DOCK_BOTTOM:
								DoInsertPane(m_panes,pane_amd,layer,row,pos+1);
								pane.Dock().Direction(wxAUI_DOCK_BOTTOM).Layer(layer).Row(row).Position(pos+1).Show();
								break;
							case wxAUI_DOCK_LEFT:
								DoInsertDockRowEx(m_panes,pane_amd,layer,row+1,pos);
								pane.Dock().Direction(wxAUI_DOCK_LEFT).Layer(layer).Row(row+1).Position(0).Show();
								break;
							case wxAUI_DOCK_CENTER:
								pane.Dock().Direction(wxAUI_DOCK_RIGHT).Layer(0).Row(GetMaxRow(m_panes,wxAUI_DOCK_RIGHT,0)+1).Position(0).Show();
								break;
					...
					case MarkPosition::MarkPosition_BOTTOM:
						{
							...
							case wxAUI_DOCK_TOP:
								DoInsertDockRowEx(m_panes,pane_amd,layer,row+1,pos);
								pane.Dock().Direction(wxAUI_DOCK_TOP).Layer(layer).Row(row+1).Position(0).Show();
								break;
							case wxAUI_DOCK_RIGHT:
								DoInsertPane(m_panes,pane_amd,layer,row,pos+1);
								pane.Dock().Direction(wxAUI_DOCK_RIGHT).Layer(layer).Row(row).Position(pos+1).Show();
								break;
							case wxAUI_DOCK_BOTTOM:
								DoInsertDockRowEx(m_panes,pane_amd,layer,row,pos);
								pane.Dock().Direction(wxAUI_DOCK_BOTTOM).Layer(layer).Row(row).Position(0).Show();
								break;
							case wxAUI_DOCK_LEFT:
								DoInsertPane(m_panes,pane_amd,layer,row,pos+1);
								pane.Dock().Direction(wxAUI_DOCK_LEFT).Layer(layer).Row(row).Position(pos+1).Show();
								break;
							case wxAUI_DOCK_CENTER:
								pane.Dock().Direction(wxAUI_DOCK_BOTTOM).Layer(0).Row(GetMaxRow(m_panes,wxAUI_DOCK_BOTTOM,0)+1).Position(0).Show();
								break;
						...
						break;
					case MarkPosition::MarkPosition_MIDDLE://tab页布局管理
						if(pane_container.HasTab())
						{
							pane.window->Reparent(pane_container.notebook);
							...
							pane_container.notebook->AddPage(pane.window,pane.caption,pane);

							DetachPane(pane.window);
						}
						else
						{
							...
							wxWindow *pParent = m_frame;
							if(pane_container.IsFloating() && pane_container.frame)
							{
								pParent = pane_container.frame;
								pane_container.frame = NULL;
							}
							wxNotebookEx *notebook = new wxNotebookEx(pParent,this);
							...
							notebook->AddPage(pane_container.window,pane_container.caption,pane_container);
							if(pane.HasTab())
							{
							...
							// detach pane
							DetachPane(pane_container.window);


							AddPane(notebook,copy_pane);
							wxAuiPaneInfo &info = m_panes.Last();
    ...
    else if (m_action == actionClickCaption)
    {
             ...
             abs(event.m_y - m_actionStart.y) > drag_y_threshold))
        {
            ...
            if (!paneInfo->IsToolbar())
            {
                ...
                {
					wxAuiFloatingFrame *float_frame = wxDynamicCast(m_frame,wxAuiFloatingFrame);
					if(float_frame)
					{
						wxAuiManager *float_mgr = float_frame->GetManager();
						float_frame->GetOwnerManager()->m_action = actionDragFloatingPane;
						m_frame->ReleaseMouse();
						// 从浮动窗口中拖出一个pane, 创建出新的浮动窗口
						// 从原浮动AUI管理器中移除paneInfo 更新主AUI管理器中paneInfo的frame,frameposition和framesize
						// TODO:
						for(int i=0,cnt=float_frame->paneWindows.GetCount();i<cnt;i++)
						{
							if(paneInfo->window == float_frame->paneWindows[i])
							{
								float_frame->paneWindows.RemoveAt(i);
								break;
							}
						}
						wxAuiPaneInfo copy_pane = *paneInfo;
						float_mgr->DetachPane(copy_pane.window);
						
						copy_pane.buttons.Clear();
						wxWindow *pParent = float_frame->GetOwnerManager()->GetManagedWindow();
						wxAuiFloatingFrame *new_float_frame = CreateFloatingFrame(pParent,copy_pane,float_frame->GetOwnerManager());
						copy_pane.window->GetContainingSizer()->Detach(copy_pane.window);
						copy_pane.window->SetContainingSizer(NULL);
						new_float_frame->SetPaneWindow(copy_pane);

						wxAuiPaneInfo& main_pane = float_frame->GetOwnerManager()->GetPane(copy_pane.window);
						main_pane.frame = new_float_frame;
						main_pane.Float().FloatingSize(float_frame->GetSize()).FloatingPosition(pParent->ScreenToClient(mouse_pos)).Show();
						copy_pane.window->Reparent(new_float_frame);
						float_frame->GetOwnerManager()->m_actionWindow = new_float_frame;
						new_float_frame->Show();
						Update();
...
//////////////////////////////////////////////////////////////////////////
//class wxAuiMarkWindow
#include "wx\aui_custom\xpm\SD2008bottom.xpm"
#include "wx\aui_custom\xpm\SD2008bottomh.xpm"
#include "wx\aui_custom\xpm\SD2008center.xpm"
#include "wx\aui_custom\xpm\SD2008centerh.xpm"
#include "wx\aui_custom\xpm\SD2008left.xpm"
#include "wx\aui_custom\xpm\SD2008lefth.xpm"
#include "wx\aui_custom\xpm\SD2008right.xpm"
#include "wx\aui_custom\xpm\SD2008righth.xpm"
#include "wx\aui_custom\xpm\SD2008top.xpm"
#include "wx\aui_custom\xpm\SD2008toph.xpm"
#include "wx\aui_custom\xpm\SD2008base.xpm"

wxBEGIN_EVENT_TABLE(wxAuiMarkWindow, wxFrame)
// EVT_MOTION(wxAuiMarkWindow::OnMotion)
EVT_CLOSE(wxAuiMarkWindow::OnClose)
EVT_PAINT(wxAuiMarkWindow::OnPaint)
EVT_ERASE_BACKGROUND(wxAuiMarkWindow::OnEraseBkg)
wxEND_EVENT_TABLE()

wxAuiMarkWindow::wxAuiMarkWindow(wxWindow* pParent,MarkPosition mark_pos)
:wxFrame(pParent, wxID_ANY, wxEmptyString,
		...
		)
{
	...
	case MarkPosition::MarkPosition_LEFT:
		{
			m_bmp = wxBitmap(SD_2008_left);
			m_btmSelected = wxBitmap(SD_2008_left_h);
		}
		break;
	case MarkPosition::MarkPosition_TOP:
		{
			m_bmp = wxBitmap(SD_2008_top);
			m_btmSelected = wxBitmap(SD_2008_top_h);
		}
		break;
	case MarkPosition::MarkPosition_RIGHT:
		{
			m_bmp = wxBitmap(SD_2008_right);
			m_btmSelected = wxBitmap(SD_2008_right_h);
		}
		break;
	case MarkPosition::MarkPosition_BOTTOM:
		{
			m_bmp = wxBitmap(SD_2008_bottom);
			m_btmSelected = wxBitmap(SD_2008_bottom_h);
		}
		break;
	case MarkPosition::MarkPosition_MIDDLE:
		{
			m_bmp = wxBitmap(SD_2008_center);
			m_btmSelected = wxBitmap(SD_2008_center_h);
		}
		break;
	}
...
void wxAuiMarkWindow::Select(bool bSel)
{
	m_bSel = bSel;
	Refresh();
}

void wxAuiMarkWindow::OnPaint(wxPaintEvent& evt)
{
	wxUnusedVar(evt);
	wxPaintDC dc(this);
	dc.DrawBitmap(m_bSel?m_btmSelected:m_bmp, 0, 0, true);
}

void wxAuiMarkWindow::OnMotion(wxMouseEvent& evt)
{
	if(wxGetMouseState().LeftIsDown())
	{
		wxRect wndRect = this->GetRect();
		if(!m_bSel && wndRect.Contains(evt.GetPosition()))
		{
			m_bSel = true;
			Refresh();
		}
...
void wxAuiMarkWindow::OnEraseBkg(wxEraseEvent &evt)
{
	wxUnusedVar(evt);
}

void wxAuiMarkWindow::OnClose(wxCloseEvent &evt)
{
	wxUnusedVar(evt);
}
//////////////////////////////////////////////////////////////////////////
//class wxAuiMarkGroupWindow
wxBEGIN_EVENT_TABLE(wxAuiMarkGroupWindow, wxFrame)
// EVT_MOTION(wxAuiMarkGroupWindow::OnMotion)
EVT_CLOSE(wxAuiMarkGroupWindow::OnClose)
EVT_PAINT(wxAuiMarkGroupWindow::OnPaint)
EVT_ERASE_BACKGROUND(wxAuiMarkGroupWindow::OnEraseBkg)
wxEND_EVENT_TABLE()

wxAuiMarkGroupWindow::wxAuiMarkGroupWindow(wxWindow* pParent)
:wxFrame(pParent, wxID_ANY, wxEmptyString,
		 wxDefaultPosition, wxDefaultSize,
		 0
		 | wxFRAME_SHAPED
		 ...
		 )
{
	...
	m_bmp_base = wxBitmap(SD_2008_base);
	m_bmp_left = wxBitmap(SD_2008_left);
	m_bmp_left_h = wxBitmap(SD_2008_left_h);
	m_bmp_top = wxBitmap(SD_2008_top);
	m_bmp_top_h = wxBitmap(SD_2008_top_h);
	m_bmp_right = wxBitmap(SD_2008_right);
	m_bmp_right_h = wxBitmap(SD_2008_right_h);
	m_bmp_bottom = wxBitmap(SD_2008_bottom);
	m_bmp_bottom_h = wxBitmap(SD_2008_bottom_h);
	m_bmp_middle = wxBitmap(SD_2008_center);
	m_bmp_middle_h = wxBitmap(SD_2008_center_h);

	//计算region
	m_rg_left = wxRegion(m_bmp_left/*,*wxBLACK*/);
	m_rg_top = wxRegion(m_bmp_top/*,*wxBLACK*/);
	m_rg_right = wxRegion(m_bmp_right/*,*wxBLACK*/);
	m_rg_bottom = wxRegion(m_bmp_bottom/*,*wxBLACK*/);
	m_rg_middle = wxRegion(m_bmp_middle/*,*wxBLACK*/);
	m_rg_base = wxRegion(m_bmp_base/*,*wxBLACK*/);
	
	m_rg_left.Offset(0,40);
	m_rg_top.Offset(40,0);
	m_rg_right.Offset(80,40);
	m_rg_bottom.Offset(40,80);
	m_rg_middle.Offset(43,43);
	m_rg_base.Offset(20,20);

	wxRegion region(0,0,0,0);
	region.Union(m_rg_left);
	region.Union(m_rg_top);
	region.Union(m_rg_right);
	region.Union(m_rg_bottom);
	region.Union(m_rg_middle);
	region.Union(m_rg_base);
	SetShape(region);

	SetTransparent(0xD0);

	wxRect rc = region.GetBox();
	SetSize(rc.width,rc.height);
}

void wxAuiMarkGroupWindow::Select(MarkPosition mark_pos,bool bSel)
{
	m_bSel = bSel;
	m_mark_pos = m_bSel?mark_pos:m_mark_pos;
}

bool wxAuiMarkGroupWindow::GetSel(MarkPosition& mark_pos)
{
	mark_pos = m_mark_pos;
	return m_bSel;
}

void wxAuiMarkGroupWindow::OnPaint(wxPaintEvent& evt)
{
	wxUnusedVar(evt);
	wxPaintDC dc(this);

	wxMemoryDC mdc(&dc);
	mdc.SelectObject(wxBitmap(GetSize()));
	//draw base
	/*dc*/mdc.DrawBitmap(m_bmp_base,20,20,true);
	//left
	/*dc*/mdc.DrawBitmap((m_bSel && m_mark_pos == MarkPosition::MarkPosition_LEFT)?m_bmp_left_h:m_bmp_left,0,40,true);
	//top
	/*dc*/mdc.DrawBitmap((m_bSel && m_mark_pos == MarkPosition::MarkPosition_TOP)?m_bmp_top_h:m_bmp_top,40,0,true);
	//right
	/*dc*/mdc.DrawBitmap((m_bSel && m_mark_pos == MarkPosition::MarkPosition_RIGHT)?m_bmp_right_h:m_bmp_right,80,40,true);
	//bottom
	/*dc*/mdc.DrawBitmap((m_bSel && m_mark_pos == MarkPosition::MarkPosition_BOTTOM)?m_bmp_bottom_h:m_bmp_bottom,40,80,true);
	//center
	if(m_bShowMiddle)
	/*dc*/mdc.DrawBitmap((m_bSel && m_mark_pos == MarkPosition::MarkPosition_MIDDLE)?m_bmp_middle_h:m_bmp_middle,43,43,true);

	dc.Blit(0, 0, GetSize().x, GetSize().y, &mdc, 0, 0);
}

void wxAuiMarkGroupWindow::OnMotion(wxMouseEvent& evt)
{
	if(wxGetMouseState().LeftIsDown())
	{
		wxPoint pt = evt.GetPosition();
		if(m_rg_left.Contains(pt) == wxRegionContain::wxInRegion)
		{
			m_bSel = true;
			m_mark_pos = MarkPosition::MarkPosition_LEFT;
		}
		else if(m_rg_top.Contains(pt) == wxRegionContain::wxInRegion)
		{
			m_bSel = true;
			m_mark_pos = MarkPosition::MarkPosition_TOP;
		}
		else if(m_rg_right.Contains(pt) == wxRegionContain::wxInRegion)
		{
			m_bSel = true;
			m_mark_pos = MarkPosition::MarkPosition_RIGHT;
		}
		else if(m_rg_bottom.Contains(pt) == wxRegionContain::wxInRegion)
		{
			m_bSel =true;
			m_mark_pos = MarkPosition::MarkPosition_BOTTOM;
		}
		else if(m_rg_middle.Contains(pt) == wxRegionContain::wxInRegion)
		{
			m_bSel = true;
			m_mark_pos = MarkPosition::MarkPosition_MIDDLE;
		}
		else
		{
			m_bSel = false;
...
void wxAuiMarkGroupWindow::OnEraseBkg(wxEraseEvent &evt)
{
	wxUnusedVar(evt);
}

void wxAuiMarkGroupWindow::OnClose(wxCloseEvent &evt)
{
	wxUnusedVar(evt);
}

bool wxAuiMarkGroupWindow::HitTest(const wxPoint &point,MarkPosition& mark_pos)
{
	if(!IsShown())
	{
		return false;
	...
	else if(m_rg_middle.Contains(pt) == wxRegionContain::wxInRegion && m_bShowMiddle)
	{
		m_bSel = true;
		m_mark_pos = MarkPosition::MarkPosition_MIDDLE;
	}
...
//////////////////////////////////////////////////////////////////////////
//class wxAuiDockMarker
wxAuiDockMarker::wxAuiDockMarker(wxAuiManager* pMgr)
{
	m_pMgr = pMgr;

	for(int i = 0; i < MarkPosition::MarkPosition_MIDDLE; i++)
	{
		m_arrMarkWnd[i] = new wxAuiMarkWindow(m_pMgr->GetManagedWindow(),(MarkPosition)(MarkPosition::MarkPosition_LEFT+i));
	}
	m_pMarkGroupWnd = new wxAuiMarkGroupWindow(m_pMgr->GetManagedWindow());
}

wxAuiDockMarker::~wxAuiDockMarker()
{

}

void wxAuiDockMarker::Show(bool bShow,bool bShowGroup)
{
	if(bShow)
	{
		//更新锚点位置
		wxWindow *pMainFrame = m_pMgr->GetManagedWindow();
		wxRect rc = pMainFrame->GetClientRect();
		rc = wxRect(pMainFrame->ClientToScreen(rc.GetLeftTop()),pMainFrame->ClientToScreen(rc.GetRightBottom()));
		for(int i=0,cnt=m_pMgr->m_panes.size();i<cnt;i++)
		{
			wxAuiPaneInfo& info = m_pMgr->m_panes[i];
			if(info.IsToolbar() && info.IsOk() && info.IsShown() && info.IsDocked())
			{
				wxRect itst_rc = info.window->GetScreenRect();
				switch(info.dock_direction)
				{
				case wxAUI_DOCK_TOP:
					rc = wxRect(wxPoint(rc.x,itst_rc.GetBottom()),rc.GetRightBottom());
					break;
				case wxAUI_DOCK_RIGHT:
					rc = wxRect(rc.GetLeftTop(),wxPoint(itst_rc.x,rc.GetBottom()));
					break;
				case wxAUI_DOCK_BOTTOM:
					rc = wxRect(rc.GetLeftTop(),wxPoint(rc.GetRight(),itst_rc.y));
					break;
				case wxAUI_DOCK_LEFT:
					rc = wxRect(wxPoint(itst_rc.GetRight(),rc.y),rc.GetRightBottom());
					break;
				case wxAUI_DOCK_CENTER: rc = wxRect();break;
				}
			}
		}
		
		wxPoint center_pt(rc.x + (rc.GetWidth()/2),rc.y + (rc.GetHeight()/2));
// 		center_pt = pMainFrame->ClientToScreen(center_pt);

		m_arrMarkWnd[MarkPosition::MarkPosition_LEFT]->SetPosition(wxPoint(center_pt.x-(rc.width/2)+10,center_pt.y-16));
		m_arrMarkWnd[MarkPosition::MarkPosition_TOP]->SetPosition(wxPoint(center_pt.x-16,center_pt.y-(rc.height/2)+10));
		m_arrMarkWnd[MarkPosition::MarkPosition_RIGHT]->SetPosition(wxPoint(center_pt.x+(rc.width/2)-46,center_pt.y-16));
		m_arrMarkWnd[MarkPosition::MarkPosition_BOTTOM]->SetPosition(wxPoint(center_pt.x-16,center_pt.y+(rc.height/2)-46));

	}
	for(int i = 0; i < MarkPosition::MarkPosition_MIDDLE; i++)
	{
		m_arrMarkWnd[i]->Show(bShow);
	}
	m_pMarkGroupWnd->Show(bShowGroup);
	if(!bShowGroup)
		m_pContainer = NULL;
}

void wxAuiDockMarker::SetContainer(wxWindow* pContainer,bool bShowMiddle,bool isMain,wxRect rc)
{
	m_pContainer = pContainer;

	m_pMarkGroupWnd->m_bShowMiddle = bShowMiddle;
	...
	if(m_pContainer)
	{
		if(isMain)
		{
			wxPoint center_pt = wxPoint(rc.x+(rc.width/2),rc.y+(rc.height/2));
			wxSize szMarkGroup = m_pMarkGroupWnd->GetSize();
			m_pMarkGroupWnd->SetPosition(wxPoint(center_pt.x-(szMarkGroup.x/2),center_pt.y-(szMarkGroup.y/2)));
			m_pMarkGroupWnd->Show();
		}
		else
		{
			wxRect rcContainer = m_pContainer->GetClientRect();
			wxPoint center_pt = m_pContainer->ClientToScreen(wxPoint(rcContainer.width/2,rcContainer.height/2));
			wxSize szMarkGroup = m_pMarkGroupWnd->GetSize();
			m_pMarkGroupWnd->SetPosition(wxPoint(center_pt.x-(szMarkGroup.x/2),center_pt.y-(szMarkGroup.y/2)));
			m_pMarkGroupWnd->Show();
		}
	...
	{
		m_pMarkGroupWnd->Show(false);
	}
}

void wxAuiDockMarker::GetDockMark(wxWindow* &pContainer,bool& bMainframe, bool& bDock, MarkPosition& mark_pos)
{
	bDock = false;
	bMainframe = false;
	for(int i = 0; i < MarkPosition::MarkPosition_MIDDLE; i++)
	{
		if(m_arrMarkWnd[i]->GetSel())
		{
			pContainer = NULL;
			bMainframe = true;
			bDock = true;
			mark_pos = (MarkPosition)(MarkPosition::MarkPosition_LEFT + i);
			return;
		}
	}
	bDock = m_pMarkGroupWnd->GetSel(mark_pos);
	if(bDock)
	{
		pContainer = m_pContainer;
	}
}

void wxAuiDockMarker::HitTest(const wxPoint& pt,wxWindow* &pContainer,bool& bMainframe, bool& bDock, MarkPosition& mark_pos,bool &isCenter,bool bRefresh)
{
	bDock = false;
	bMainframe = false;
	for(int i = 0; i < MarkPosition::MarkPosition_MIDDLE; i++)
	{
		if(m_arrMarkWnd[i]->GetRect().Contains(pt))
		{

			m_arrMarkWnd[i]->Select(true);
		...
		{
			m_arrMarkWnd[i]->Select(false);
		}
	}
	bDock = m_pMarkGroupWnd->HitTest(pt,mark_pos);
	if(bDock)
	{
		isCenter = true;
		pContainer = m_pContainer;
		if(m_pContainer == m_pMgr->m_frame)
			bMainframe = true;
	}
}
...
//////////////////////////////////////////////////////////////////////////
//class wxNotebookEx
wxBEGIN_EVENT_TABLE(wxNotebookEx, wxNotebook)
	EVT_LEFT_DOWN(wxNotebookEx::OnLeftDown)
	EVT_LEFT_DCLICK(wxNotebookEx::OnLeftDClick)
	EVT_LEFT_UP(wxNotebookEx::OnLeftUp)
	EVT_MOTION(wxNotebookEx::OnMotion)
	EVT_MOUSE_CAPTURE_LOST(wxNotebookEx::OnCaptureLost)
wxEND_EVENT_TABLE()

wxNotebookEx::wxNotebookEx(wxWindow* parent,wxAuiManager* mgr)
:wxNotebook(parent,wxID_ANY,wxDefaultPosition,wxDefaultSize,wxNB_BOTTOM),m_mgr(mgr)
{
	pane_info = NULL;
	index = wxNOT_FOUND;
}

bool wxNotebookEx::AddPage(wxWindow *page, const wxString& text, wxAuiPaneInfo& pane, bool bSelect, int imageId)
{
	bool res = wxNotebook::AddPage(page,text,bSelect,imageId);
	if(res)
		page_panes.Add(pane);
	return res;
}

void wxNotebookEx::OnLeftDown(wxMouseEvent& evt)
{
	last_pos = evt.GetPosition();

	long idx = wxNOT_FOUND,flag = 0;
	idx = HitTest(last_pos,&flag);
	if(idx>wxNOT_FOUND && (flag&(wxBK_HITTEST_ONICON|wxBK_HITTEST_ONITEM|wxBK_HITTEST_ONLABEL)))
	{
		index = idx;
// 		CaptureMouse();
	}
...
void wxNotebookEx::OnLeftDClick(wxMouseEvent& evt)
{
	wxPoint pt = wxGetMousePosition();
	...
	if(idx>wxNOT_FOUND && (flag&(wxBK_HITTEST_ONICON|wxBK_HITTEST_ONITEM|wxBK_HITTEST_ONLABEL)))
	{
		wxAuiPaneInfo info = page_panes[idx];
		info.buttons.Clear();
		wxRect nb_rect = pane_info->window->GetScreenRect();
		info.FloatingPosition(nb_rect.x+(nb_rect.GetWidth()/2),nb_rect.y+(nb_rect.GetHeight()/2)).Float().Show();
		
		info.window = GetPage(idx);
		info.window->Reparent(m_mgr->GetManagedWindow());
		
		RemovePage(idx);
		page_panes.RemoveAt(idx);

		wxAuiManager *mgr = m_mgr;
		// 只有一个tab page 取消 tab管理
		if(GetPageCount()<=1)
		{
			wxAuiPaneInfo last_info = page_panes[0];
			last_info.window = GetPage(0);
			last_info.window->Reparent(mgr->m_frame);
			last_info.buttons.Clear();
			last_info.Dock().Position(pane_info->dock_pos)/*.BestSize(pane_info->best_size)*/
				.Direction(pane_info->dock_direction).Layer(pane_info->dock_layer).Show();
			
			mgr->DetachPane(this);
			RemovePage(0);
			Destroy();

			mgr->AddPane(last_info.window,last_info);
		}

		mgr->AddPane(info.window,info);
		mgr->Update();
	}
	return;
}
void wxNotebookEx::OnLeftUp(wxMouseEvent& evt)
{
// 	ReleaseMouse();
	return;
}

void wxNotebookEx::OnMotion(wxMouseEvent& evt)
{
	if(evt.Dragging())
	{
		wxPoint pt = evt.GetPosition();
		wxPoint srceen_pt = ClientToScreen(pt);
		...
		// is now being dragged. if it is, we need to change the
		// mouse action to 'drag'
			if((abs(pt.x - last_pos.x) > drag_x_threshold ||
				abs(pt.y - last_pos.y) > drag_y_threshold) && index!=wxNOT_FOUND)
		{
			wxAuiPaneInfo info = page_panes[index];
			info.window = GetPage(index);
			info.window->Reparent(m_mgr->m_frame);
			info.buttons.Clear();
			info.notebook = NULL;

			page_panes.RemoveAt(index);
			RemovePage(index);
			...
			if(GetPageCount()<=1)
			{
				wxAuiPaneInfo last_info = page_panes[0];
				last_info.window = GetPage(0);
				last_info.window->Reparent(GetParent());
				last_info.buttons.Clear();
				last_info.notebook = NULL;

				wxAuiFloatingFrame *pFrame = wxDynamicCast(GetParent(),wxAuiFloatingFrame);
// 				if(pane_info->frame && pane_info->IsFloating())
// 				{
// 					wxAuiPaneInfo& main_pane = mgr->GetPane(this);
// 					last_info.Float()
// 						.FloatingPosition(main_pane.floating_pos)
// 						.FloatingSize(main_pane.floating_size).Show();
// 					mgr->AddPane(last_info.window,last_info);
// 				}
// 				else
// 				{
...
// 				}
				if(pFrame)
				{
					wxAuiPaneInfo& main_pane = pFrame->GetManager()->GetPane(this);
					main_pane.window = last_info.window;
					main_pane.window->Reparent(pFrame);
					main_pane.notebook = NULL;
					main_pane.name = last_info.name;
					main_pane.caption = last_info.caption;

					pFrame->GetManager()->Update();

					mgr->DetachPane(this);
					RemovePage(0);
					mgr->AddPane(last_info.window,last_info.Float().FloatingPosition(pFrame->GetPosition()).FloatingSize(pFrame->GetSize()).CaptionVisible().PaneBorder());
			...
			mgr->m_frame->CaptureMouse();
			mgr->m_action = wxAuiManager::actionDragFloatingPane;
			info.Float().FloatingPosition(srceen_pt).Show();
			mgr->AddPane(info.window,info);
			wxAuiPaneInfo &refInfo = mgr->m_panes.Last();
			mgr->Update();
			mgr->m_actionWindow = refInfo.frame;
		}
	}
	return;
}

void wxNotebookEx::OnCaptureLost(wxMouseCaptureLostEvent& evt)
{
	return;
}

bool wxNotebookEx::MSWOnNotify(int idCtrl, WXLPARAM lParam, WXLPARAM *result)
{
	NMHDR* hdr = (NMHDR *)lParam;
	switch ( hdr->code ) {
	case TCN_SELCHANGE:
		UpdateSelection(TabCtrl_GetCurSel(GetHwnd()));
		if(pane_info->frame && pane_info->IsFloating())
			pane_info->frame->SetTitle(GetPageText(TabCtrl_GetCurSel(GetHwnd())));
		else
		{
			m_mgr->GetPane(wxDynamicCast(this,wxWindow)).caption = GetPageText(TabCtrl_GetCurSel(GetHwnd()));
			m_mgr->Update();
		}
		return true;
		break;
